...
public class ArrayListExample {
	...
	public static void main(String[] args) {
		//<Board> => "제네릭"     배열에 저장할 타입을 명시, 뒤에 오는건 타입을 생략해도 상관없다.
...
//		Board board1 = new Board("제목1", "내용1", "글쓴이1");
		...
		//특정 인덱스 번호의 객체 가져오기
		Board board = list.get(2); //인덱스 번호가 2인 board 객체를 리턴 해주므로 Board 타입으로 받는다.
		System.out.println(board.getSubject() + " " + board.getContent() + " " + board.getWriter());
		System.out.println();
		
		//모든 객체를 하나씩 가져오기
		for(int i=0; i<list.size(); i++) {
			Board b = list.get(i); //인덱스 번호가 2인 board 객체를 리턴 해주므로 Board 타입으로 받는다.
