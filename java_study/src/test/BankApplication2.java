...
//		}
	...
	private static void deposit() {
		...
		int money = scanner.nextInt(); //String타입으로 입력을 받는다.
	...
	private static Account2 findAccount(String ano) {
		Account2 resultAccount = null;
		for(Account2 account : accountArray) {
//			배열의 값이 null이 아닐때만 실행한다. => account 객체를 가지고 있을때
			if(account != null) {
				if (ano.equals(account.getAno())) {
//					여기서 리턴하면 안되므로 아래와 같이 계좌를 넣어준다.
					resultAccount = account;
					break;
				}
			}
		}	
		return resultAccount; //찾은 계좌를 리턴
	}
